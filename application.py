...
def index():
    ...
    message=Markup("""
    <p style="text-align: center; color: rgb(63,102,101); font-size:39px;"   > <b>Welcome to the website! </b></p>
    <blockquote class="blockquote p-1 mt-5">
    <p style="text-align: center; color: rgb(135,127,146); font-size:27px;"   >“A reader lives a thousand lives before he dies, said Jojen. <br>The man who never reads lives only one.”</p>
    <p style="text-align: center;  font-size:24px;"   > <b>                                                                 George R.R. Martin, A Dance with Dragons </b></p>
  </blockquote>""")
    session["books"]=[]   
...
def api(isbn):
    ...
    res = requests.get("https://www.goodreads.com/book/review_counts.json", params={"key": "4F8jNRtlEyCAZDu4iOVsw", "isbns": isbn})
    average_rating=res.json()['books'][0]['average_rating']
    work_ratings_count=res.json()['books'][0]['work_ratings_count']
    x = {
    "title": data.title,
